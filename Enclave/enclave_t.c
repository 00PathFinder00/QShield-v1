...
static sgx_status_t SGX_CDECL sgx_enclaveAdd(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_enclaveAdd_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_enclaveAdd_t* ms = SGX_CAST(ms_enclaveAdd_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = enclaveAdd(ms->ms_a, ms->ms_b);
...
SGX_EXTERNC const struct {
	...
	2,
	{
		...
		{(void*)(uintptr_t)sgx_enclaveAdd, 0},
	}
};
