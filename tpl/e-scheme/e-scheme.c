...
// sgx_status_t eencrypt(uint8_t* msg, uint32_t msg_size, uint8_t *ciphers, uint8_t *cipher_gcm_mac, element_t sk){
//   sgx_status_t ret = SGX_SUCCESS;
...
//   ret = sgx_rijndael128GCM_encrypt(&u_key, msg, msg_size, &ciphers[0], &aes_gcm_iv[0], 12, NULL, 0,
//     (sgx_aes_gcm_128bit_tag_t *)(cipher_gcm_mac));
//   return ret;
// }

// bool isSame(sgx_sha256_hash_t* skbi_gti_tag_user,sgx_sha256_hash_t* skbi_gti_tag_ska){
...
// }
//
// sgx_status_t edecrypt(uint8_t* cipher, uint32_t cipher_size, uint8_t *msgs, uint8_t *ct_gcm_mac, char *policy, e_ska* ska, element_t skbi){
// /** compute skbi_tag
//  */
//  sgx_status_t ret = SGX_SUCCESS;
//  sgx_sha256_hash_t skbi_tag;
//  uint32_t skbi_len=element_length_in_bytes(skbi);
//  uint8_t skbi_str[skbi_len];
//  element_to_bytes(skbi_str, skbi);
//  sgx_sha_state_handle_t sha_context;
//  ret = sgx_sha256_init(&sha_context);
//
//  if (ret != SGX_SUCCESS)
...
//   bool flag=false;
//   element_t gti;
//   element_init_G1(gti,ska->pairing);
//   for (size_t i = 0; i< USER_NUM; i++) {
//   flag=isSame(&skbi_tag, &ska->comps[i].skbi_tag);
//   if (flag) {
//   element_set(gti,ska->comps[i].gti);
//   break;
//   }
//   }
//   if (!flag) {
...
//   /** get sk*/
//   element_t tmp1,tmp2;
//   element_init_GT(tmp1,ska->pairing);
//   element_init_GT(tmp2,ska->pairing);
//   pairing_apply(tmp1,gti,skbi,ska->pairing);
//   element_mul(tmp2,ska->sk_a,ska->sk_a);
//   element_t sk;
//   element_init_GT(sk,ska->pairing);
//   element_div(sk,tmp2,tmp1);
//
//
//   /**decrypt  data */
//   int len=element_length_in_bytes(sk);
//   uint8_t sk_str[len];
//   element_to_bytes(sk_str, sk);
//   uint8_t u_key[16];
//   uint8_t aes_gcm_iv[12] = {0};
//   strncpy(u_key,sk_str,16);
//   ret = sgx_rijndael128GCM_decrypt(&u_key, cipher, cipher_size, &msgs[0], &aes_gcm_iv[0], 12, NULL, 0, (const sgx_aes_gcm_128bit_tag_t *)(ct_gcm_mac));
//   return ret;
// }
